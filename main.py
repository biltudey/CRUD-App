...
def index():
    if request.method == 'POST':
        ...
        try:
            ...
            return 'we are facing an issue please try later'
    else:
        task = Todo.query.order_by(Todo.date_created).all()
        return render_template('index.html',tasks = task,font_url='https://fonts.googleapis.com/css2?family=Syne&display=swap') #font
